# NasTech Workspace — Comprehensive Features Inventory

> **Version:** 2.0.0 | **Stack:** React 19 + TanStack Start/Router + Vite 7 + Tailwind CSS 4 + Zustand + xterm.js + Monaco Editor  
> **Description:** Desktop workspace for NasTech Agent — chat, orchestration, and multi-agent coding pipelines
...
- **Dual chat backend modes:**
  - **Enhanced Claude** — full session API with persistent history via NasTech Agent gateway
...
- **Browse agent memory files** in `~/.nastech/` (MEMORY.md, memory/, memories/)
...
| Endpoint             | Method | Description                      |
| -------------------- | ------ | -------------------------------- |
| `/api/memory`        | GET    | Get memory from NasTech Agent gateway   |
...
| `/api/claude-config` | GET    | Read NasTech config.yaml and .env             |
...
| `/api/start-claude`      | POST   | Start NasTech Agent gateway                          |
...
| ------------------------- | ------------------------------- | -------- | --------------------------- |
| `claudeUrl`               | string                          | `''`     | NasTech Agent API URL              |
...
| ---------------------- | -------------------------------------------------- |
| `NASTECH_API_URL`       | Backend API URL (default: `http://127.0.0.1:8642`) |
| `CLAUDE_PASSWORD`      | Optional password protection for web UI            |
| `CLAUDE_WORKSPACE_DIR` | Workspace root directory (default: `~/.nastech`)    |
| `NASTECH_AGENT_PATH`    | Path to nastech-agent directory                     |
...
| `NASTECH_API_TOKEN`     | Auth token for gateway API_SERVER_KEY              |
| `BEARER_TOKEN`         | Bearer token for backend auth                      |
| `PORT`                 | Server port (default: 3002 dev, 3000 prod)         |

### 4.6 Claude Config Management

- **Read/write `~/.nastech/config.yaml`** — YAML config via web UI
- **Read/write `~/.nastech/.env`** — environment variables
- **Provider status** with masked API keys
- **Auth store integration** — reads from `~/.nastech/auth-profiles.json` and `~/.openclaw/agents/main/agent/auth-profiles.json`
...
- Persisted run state at `~/.nastech/webui-mvp/runs/`
...
- Filesystem-based memory browsing in `~/.nastech/`
...
### 5.7 NasTech Agent Auto-Start

- Auto-detects sibling `nastech-agent/` directory
- Resolves Python virtualenv (`.venv`, `venv`, system `python3`)
- Spawns uvicorn with health polling (15 attempts, 1s interval)
- Reads `~/.nastech/.env` for agent configuration
...
- Auth profile storage in `~/.nastech/auth-profiles.json`
...
- **nastech-agent** container — Python FastAPI gateway on port 8642
- **nastech-workspace** container — Node.js web UI on port 3000
...
_Generated from codebase analysis of `/Users/aurora/nastech-workspace/`_
