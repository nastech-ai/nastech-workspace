...
function classifyConnectionError(
  ...
  if (!normalizedError && !status) {
    return {
      title: 'Not connected',
      description: "NasTech Workspace can't reach NasTech Agent.",
      action: 'Check that NasTech is running, then try again.',
    }
  }
  ...
  if (
    ...
    return {
      ...
      description: 'NasTech Agent rejected the connection token.',
      action: 'Go to Settings -> Advanced -> NasTech Agent to update your token.',
    }
  }
  ...
  if (
    ...
    return {
      ...
      description: "This device isn't paired with NasTech Agent yet.",
      action: 'Check NasTech Agent connection.',
    }
  }

  if (lower.includes('econnrefused') && lower.includes('8642')) {
    return {
      title: 'NasTech Agent gateway not running',
      description: 'The NasTech Agent gateway is not running on port 8642.',
      action: 'Run the official NasTech installer, then start the gateway with: nastech gateway run',
    }
  }
  ...
  if (
    ...
    return {
      title: 'NasTech Agent unreachable',
      description: "Can't connect to NasTech Agent at the configured URL.",
      action: 'Make sure NasTech is running and the URL is correct.',
    }
  }
  ...
  return {
    ...
    action: 'Try refreshing or check Settings -> Advanced -> NasTech.',
  }
}
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 text-xs">
          <p className="font-medium">
            {isChecking ? 'Connecting to NasTech Agent...' : errorInfo.title}
          </p>
          {!isChecking ? (
            <>
