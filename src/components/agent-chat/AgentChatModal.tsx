...
function buildDemoReply(agentName: string, text: string): string {
  return `${agentName} (demo): Received "${text}". NasTech Agent is unavailable, so this is a simulated response.`
}
          ...
          setMessages([
            {
              ...
              role: 'agent',
              text: 'NasTech Agent is unavailable. Running in demo mode with simulated responses.',
              timestamp: Date.now(),
            },
          ])
        }
