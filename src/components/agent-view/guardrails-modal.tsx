...
export function GuardrailsModal({ open, agentName, agentId, sessionKey, onOpenChange }: GuardrailsModalProps) {
  ...
  useEffect(() => {
    ...
    const parsed = normalizedId
      ? parseGuardrails(window.localStorage.getItem(`nastechsuite:guardrails:${normalizedId}`))
      : DEFAULT_CONFIG
  ...
  function handleSave() {
    ...
    window.localStorage.setItem(`nastechsuite:guardrails:${normalizedId}`, JSON.stringify(config))
