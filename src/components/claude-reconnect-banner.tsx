...
async function probeClaudeHealth(): Promise<boolean> {
  ...
  // which works with both NasTech Agent and OpenAI-compatible backends.
  try {
              ...
              void fetch('/api/start-claude', {
                ...
                .then(async (res) => {
                  ...
                  if (res.ok && data.ok) {
                    setMessage(
                      data.message ||
                        'Auto-restarting NasTech Agent gateway…',
                    )
  ...
  async function handleStartAgent(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(payload.error || 'Failed to start NasTech Agent')
      }

      setMessage(
        payload.message === 'already running'
          ? 'NasTech Agent is already running'
          : 'Starting NasTech Agent…',
      )
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : 'Failed to start NasTech Agent',
      )
    } finally {
      setIsStarting(false)
      await probeNowRef.current?.(true)
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold">
              {isDisconnected ? 'NasTech Agent not connected' : 'Connected'}
            </p>
            {message ? (
