...
| # | Beat | Time | Notes |
|---|---|---|---|
| 1 | Title screen — show **NasTechWorld** gold serif title, "the agent MMO" tagline, premium gold "Enter the Realm" CTA | 0:00 – 0:08 | Type your name. Click Customize Avatar briefly to show the Sims-lite picker, then close. |
| 2 | Click "Enter the Realm" → land in **Training Grounds**. Narration auto-plays (~12s). | 0:08 – 0:18 | Slow camera orbit on the central **NasTech statue** with stone-tile plaza, braziers, banners, and practice dummies in the Trainer's Ring. |
...
| 4 | Talk to Athena → accept first quest → claim NasTech Sigil. | 0:25 – 0:32 | Quest tracker on the right ticks. |
...
| 6 | Press **4 (Summon)** — a glowing purple familiar orbits you. (NasTech Summoning skill.) | 0:42 – 0:48 | "Summoner of the Forge" title unlocks if you trigger this in the Forge. |
...
| 8 | Visit **Archive Podium** (Iris) for the docs/memory beat. | 0:54 – 0:60 | Modal explains NasTech Memory in plain English. |
| 9 | Walk to **Forge Gate** with banners + brazier. Step through. Forge intro plays. | 0:60 – 0:70 | Cyan-flame braziers + NasTech statue + floating data motes. |
...
| 12 | End on slow zoom-out from the **NasTech statue** in Training Grounds with banners + braziers + floating sparkles + your knight avatar. | 0:88 – 0:95 | Title overlay or just let it breathe. |
...
1. **Title screen**: "NasTech Workspace turns agent onboarding into a multiplayer RPG. This is **NasTechWorld** — a 3D realm where you don't read about NasTech, you play it."
2. **Training Grounds**: "Six worlds. Each one teaches a real NasTech Agent skill. Movement, equipment, chat, memory, summoning, diplomacy."
...
5. **Knight gear**: "Your gear maps to your NasTech loadout. Each piece changes both your stats and your silhouette in the world."
6. **Multiplayer**: "Real multiplayer over Cloudflare Durable Objects. Other NasTech builders walking the same Agora as you. Chat, presence, world-scoped fan-out."
7. **End**: "NasTechWorld. The fastest way to teach builders what NasTech Agent actually feels like."

---

## What's covered (NasTech 6 Skills audit)
...
| | |
|---|---|
| Title screen — NasTechWorld branding, gold serif, premium feel | ✅ |
| Mouse drag camera (left/right/middle) + wheel zoom | ✅ |
| Avatar customizer (Sims-lite) | ✅ |
| Knight armor on local + remote players | ✅ |
| NasTech statue + braziers + banners (Training, Agora, Forge, Grove, Oracle, Arena) | ✅ |
...
| 5-step NasTech tutorial chain + 2 bonus quests (Wisp, Diplomacy, Summoning) | ✅ |
...
gh pr create \
  --title "NasTechWorld: agent MMO with full multiplayer" \
  --body "$(cat memory/goals/2026-05-03-playground-training-grounds/NASTECHWORLD-FINAL-DEMO-SCRIPT.md)"
```
...
| Player avatar disappears | Check `__nastechPlaygroundLiveCount` in console — server prune is now 12s, hibernation API survives bg-tabs. Should be solid. |
...
| WS hub down | `curl https://nastech-playground-ws.myaurora-agi.workers.dev/stats` |
