...
- A first-time visitor on `/playground` no longer needs NasTech Agent running, no API key, no Codex auth.
- Tutorial Q1–Q5 already advance via canned dialog choices (verified in earlier iterations).
- If a visitor *does* have a healthy local NasTech gateway and sets `VITE_PLAYGROUND_LLM_CHAT=1`, free-form chat lights up automatically and gracefully degrades to scripted mode on any provider failure.
...
- Decide on free-tier hosted model fallback (Kimi free / Nous Portal / OpenRouter free) for users who want live LLM but don't run NasTech locally.
