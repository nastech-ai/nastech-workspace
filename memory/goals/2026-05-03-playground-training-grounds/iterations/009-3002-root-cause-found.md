...
**Three concurrent `vite dev` processes were running against the same `nastech-workspace` source tree.**

Process inventory at start of session:
- pid 40934 — vite dev on **:3001** (running 36+ min, cwd `/Users/aurora/nastech-workspace`)
...
- pid 71649 — vite dev on **:3005** (different cwd: `/Users/aurora/.worktrees/nastech-playground-local`) — **fine**
- pid 11433 — vite dev on **:3006** (worktree, fine)

All three duplicate `nastech-workspace` vite servers were running **TanStack Router's
...
- 34,039 instances of "modified by another process" in `/tmp/nastech3002.log`
...
1. Killed the three duplicate nastech-workspace vite processes.
...
If more than one vite is bound to the same source tree (`/Users/aurora/nastech-workspace`),
that is the bug. Worktree vites at different cwds are fine.

A single command that detects it:

```bash
ps aux | grep -E "vite.*nastech-workspace/node_modules" | grep -v grep | grep -v worktree | wc -l
```
