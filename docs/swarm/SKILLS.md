...
| `swarm-ui-worker` | UI implementation lane for NasTech Workspace surfaces. |
...
When you clone the repo and run the workspace, worker sessions load skills from the configured NasTech Agent profile. In Eric's release environment, profiles point at a shared skills directory, commonly exposed to workers as:
...
```text
~/.nastech/profiles/<workerId>/skills/
```
...
Base contract for every NasTech Agent:
