...
NasTech Agent sessions fill context extremely fast during normal tool-heavy workflows. In one Workspace session (`cda915a9-fbf6-4bbf-9617-e7e9d26f40be`):
...
### Option A — Canonical NasTech Agent artifact store

Best long-term because all clients benefit: CLI, gateway, Workspace, future WebUI.

Add a table/store to NasTech Agent:
...
```txt
~/.nastech/sessions/artifacts/<session_id>/<artifact_id>.json
```
...
```txt
~/nastech-workspace/.runtime/artifacts/<session_id>/<artifact_id>.json
```
...
Good MVP, but not enough for canonical model-context bloat unless NasTech Agent context builder also stops reinjecting the full tool payload.
...
## Current Diagnosis: Workspace vs NasTech Agent

The trailing “Tool work completed” card is a Workspace guardrail, not the desired steady state.

Likely responsibility split:

1. **NasTech Agent / session persistence is the root source** when stored history ends with tool messages and no final assistant text. A healthy agent transcript should end each tool-using turn with one user-visible assistant outcome: final answer, error, interrupted/cancelled, no-op, or compact tool-work summary.
...
Conclusion: Workspace should defensively collapse/tool-summary this state, but the canonical fix belongs in NasTech Agent’s stream/session writer: never persist a completed chat turn that ends only with tool output.

## NasTech Agent Fix Implemented

Patched `/Users/aurora/nastech-agent`:
...
2. Continue canonical artifact work in NasTech Agent: externalize oversized tool outputs before they enter persisted/model context, not only before Workspace renders history.
