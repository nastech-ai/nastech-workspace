...
## NasTech Workspace — Profile-Parallel Agent Execution
...
NasTech Workspace currently operates as a **single-gateway, single-profile UI**. The gateway loads one `NASTECH_HOME` at startup and all chat sessions, operations, and memory access flow through that one process.
...
**Core truth**: Each NasTech profile is a **distinct cognitive agent** — different SOUL.md, different skills, different memory, different purpose. They are not "modes" of one agent. They are parallel agents.

**Implication**: The workspace must be an **agent orchestrator**, not just a UI skin over one gateway.

**Constraint**: NasTech Agent gateway is designed as a single-tenant process. It cannot dynamically reload profiles. Each profile needs its own gateway instance.
...
**Profile-count agnostic**: The pool manager discovers profiles dynamically from the filesystem (`~/.nastech/profiles/*`). There is no hardcoded list, no maximum count, and no special-casing of specific profile names. A user with 2 profiles and a user with 50 profiles use the exact same code path.
...
**Note**: The gateway must be spawned via `nastech gateway`, not via the workspace's internal gateway.ts. The workspace becomes an orchestrator, not a gateway host.
...
- Default to `activeProfile` (from `~/.nastech/active_profile` file)
...
Currently: All sessions in `~/.nastech/sessions/` (or profile's sessions dir)
...
- No cross-profile memory leakage (each gateway has its own `NASTECH_HOME`)
...
- PR #118: Profile-aware config (merged) — provides `NASTECH_HOME` resolution and profile listing
...
*Authored by Nous (Vivere Vitalis) for the NasTech Workspace project.*
