...
NasTech Workspace + NasTech Agent in containers.
...
- `nastech-agent` (port `8642`, internal only)
- `nastech-workspace` (port `3000`, bound to `127.0.0.1`)
...
```bash
NASTECH_API_URL=http://<agent-host-or-service>:8642
NASTECH_API_TOKEN=<the same value as API_SERVER_KEY>
NASTECH_DASHBOARD_URL=http://<agent-host-or-service>:9119
NASTECH_DASHBOARD_TOKEN=<same key, or set CLAUDE_DASHBOARD_TOKEN>
```

Inside docker compose on the same host, `<agent-host-or-service>` is the service name from your compose file (e.g. `nastech-agent`). On a Synology NAS with a separate workspace stack, it's the LAN IP (e.g. `192.168.1.78`).
...
```bash
NASTECH_PASSWORD=<a long random string different from API_SERVER_KEY>
```
...
If the workspace shows "**Disconnected**" or "**Missing NasTech APIs detected**" but the agent appears to be running:

### Step 1 — Verify the agent is reachable from inside the workspace container

```bash
docker compose exec nastech-workspace sh
# inside the workspace container:
curl -fsS http://nastech-agent:8642/health
curl -fsS -H "Authorization: Bearer $NASTECH_API_TOKEN" http://nastech-agent:8642/v1/models | head -c 200
exit
```
...
```bash
docker compose exec nastech-workspace env | grep -E "NASTECH_API|API_SERVER"
```

You should see:

- `NASTECH_API_URL=http://nastech-agent:8642` (or whichever service name)
- `NASTECH_API_TOKEN=<same value as agent's API_SERVER_KEY>`
...
```bash
docker compose logs nastech-workspace 2>&1 | grep '\[gateway\]' | tail -3
```

A healthy log looks like:

```
[gateway] gateway=http://nastech-agent:8642 dashboard=http://nastech-agent:9119 mode=zero-fork core=[health,chatCompletions,models,streaming] enhanced=[sessions,skills,memory,config,jobs,enhancedChat,conductor,kanban] missing=[mcp]
```

A failing log usually shows `core=[]` and `missing=[health,...]` — that means every probe got a non-2xx response. Check the agent's logs (`docker compose logs nastech-agent`) for matching 401/404/timeout entries.
...
| All probes 401 | `NASTECH_API_TOKEN` doesn't match agent's `API_SERVER_KEY` | Check both `.env` values are the same. They must match exactly. |
| Workspace UI shows "Connection refused" | Workspace using `127.0.0.1` instead of the service name | Set `NASTECH_API_URL=http://nastech-agent:8642` (or whichever service name). |
...
```bash
NASTECH_API_URL=http://192.168.1.78:8642
NASTECH_API_TOKEN=<API_SERVER_KEY>
NASTECH_DASHBOARD_URL=http://192.168.1.78:9119
```
...
## NasTech Workspace + NasTech Agent: why two containers?

The workspace is the **UI**. The agent is the **engine**. Splitting them lets you:

- Update either independently (`docker compose pull nastech-workspace` etc.)
...
If your setup matches the playbook above and still breaks, file an issue at <https://github.com/nastech-ai/nastech-workspace/issues> with:

1. Your `docker-compose.yml` (redact secrets)
2. The output of `docker compose logs nastech-workspace 2>&1 | grep '\[gateway\]' | tail -5`
